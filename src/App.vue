...
<template>
<div class="h-screen flex text-slate-200 font-sans antialiased bg-slate-950 overflow-hidden selection:bg-indigo-500 selection:text-white">
    <!-- Sidebar -->
    <aside class="w-20 lg:w-64 border-r border-slate-800 flex flex-col justify-between bg-slate-900/50 transition-all duration-300 hidden md:flex">
        <div>
    ...
    <!-- Mobile Nav (Bottom) -->
    <nav class="md:hidden fixed bottom-0 left-0 right-0 bg-slate-900 border-t border-slate-800 p-2 z-40 flex justify-around">
        <button class="p-2 text-slate-400" :class="currentView==='focus' ? 'text-indigo-400' : ''" @click="currentView='focus'"><icon-lucide name="target" size="24"></icon-lucide></button>
        <button class="p-2 text-slate-400" :class="currentView==='calendar' ? 'text-indigo-400' : ''" @click="currentView='calendar'"><icon-lucide name="calendar" size="24"></icon-lucide></button>
        <button class="p-2 text-slate-400" :class="currentView==='timeless' ? 'text-indigo-400' : ''" @click="currentView='timeless'"><icon-lucide name="infinity" size="24"></icon-lucide></button>
    </nav>
